	  ...
	  m_adv(ourNode)
{
	...
	Power.Supported(ourNode.Info.NodeType() != UNKNOWN && ourNode.Info.FirmwareVersionCode.Value() >= FW_MILESTONE_PWR);
}
...
void CPMnode::Refresh() 
{
	if ((Port.OpenState() == OPENED_ONLINE || Port.OpenState() == OPENED_SEARCHING) 
		&& Port.NodeCount() > nodeaddr(NODE_ADDR(Info.Ex.Addr()))) {
		// Setup advanced API items
		optionReg myOptions;
		...
		Status.Power.Supported(Info.FirmwareVersionCode.Value() >= FW_MILESTONE_PWR);
		// Update unit scalings
		AccUnit(m_AccUnit);
		VelUnit(m_VelUnit);
		TrqUnit(m_TrqUnit);
	}
	else {
		m_info.NodeType(IInfo::UNKNOWN);
	}
}
